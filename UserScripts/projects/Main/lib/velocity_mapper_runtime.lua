...
-- Apply curve shaping to a normalized input (0..1)
-- curve: 0 = linear, 1 = soft (exponential), 2 = hard (logarithmic-like)
local function applyCurve(normalized, curve)
  local n = clamp(normalized, 0.0, 1.0)
  local c = math.floor(tonumber(curve) or 0)
  if c == 1 then
    -- soft: exponential-ish (gentler at low values)
    return n * n * (3.0 - 2.0 * n)
  elseif c == 2 then
    -- hard: steeper at low values
    return n * n
  else
    -- linear
    return n
...
-- Remap amplitude value based on amount, curve, and offset
-- amount: 0..1 (0 = no effect, 1 = full effect)
-- curve: 0=linear, 1=soft, 2=hard
-- offset: -1..1 (bipolar offset applied after shaping)
local function remapAmplitude(amp, amount, curve, offset)
  local a = clamp(amp, 0.0, 1.0)
  ...
  -- Apply curve shaping
  local shaped = applyCurve(a, curve)
  
  -- Blend between original and shaped based on amount
  local blended = a * (1.0 - amt) + shaped * amt
  
  -- Apply offset (scaled by amount so offset has no effect when amount is 0)
  local result = blended + off * amt
  
  return clamp(result, 0.0, 1.0)
end

function M.ensureDynamicRuntime(ctx)
  ctx._dynamicVelocityMapperRuntime = ctx._dynamicVelocityMapperRuntime or {}
  _G.__midiSynthDynamicVelocityMapperRuntime = ctx._dynamicVelocityMapperRuntime
  _G.__midiSynthVelocityMapperViewState = _G.__midiSynthVelocityMapperViewState or {}
  return ctx._dynamicVelocityMapperRuntime
end
...
function M.resolveModuleState(ctx, moduleId, voiceCount)
  ...
  state = {
    ...
    values = { amount = 1.0, curve = 0, offset = 0.0 },
    inputs = {},
...
function M.refreshModuleParams(ctx, state, readParam)
  ...
  state.values.amount = clamp(readParam(paramBase .. "/amount", state.values.amount or 1.0), 0.0, 1.0)
  state.values.curve = math.floor(clamp(readParam(paramBase .. "/curve", state.values.curve or 0), 0, 2) + 0.5)
  state.values.offset = clamp(readParam(paramBase .. "/offset", state.values.offset or 0.0), -1.0, 1.0)
  return state.values
end

local function currentParams(state)
  if type(state) ~= "table" then
    return 1.0, 0, 0.0
  end
  local values = state.values or {}
  local amount = tonumber(values.amount) or 1.0
  local curve = math.floor(tonumber(values.curve) or 0)
  local offset = tonumber(values.offset) or 0.0
  return clamp(amount, 0.0, 1.0), math.floor(clamp(curve, 0, 2)), clamp(offset, -1.0, 1.0)
end
...
function M.resolveVoiceBundleSample(ctx, sourceId, sourceEndpoint, voiceIndex, clampFn)
  ...
  if sourceKey ~= "velocity_mapper.voice" and specId ~= "velocity_mapper" then
    return nil
  end
  ...
  local mapped = copyVoicePayload({}, input)
  
  -- Remap amplitude fields while preserving note/gate/source
  mapped.amp = remapAmplitude(input.amp or 0.0, amount, curve, offset)
  mapped.targetAmp = remapAmplitude(input.targetAmp or input.amp or 0.0, amount, curve, offset)
  mapped.currentAmp = remapAmplitude(input.currentAmp or input.amp or 0.0, amount, curve, offset)
  
  -- Preserve all other fields exactly
  mapped.note = input.note
  mapped.gate = input.gate
  mapped.noteGate = input.noteGate
  mapped.envelopeLevel = input.envelopeLevel
  mapped.envelopeStage = input.envelopeStage
  mapped.active = input.active
  mapped.sourceVoiceIndex = input.sourceVoiceIndex
...
function M.resolveVoiceModulationSource(ctx, sourceId, source, voiceCount)
  ...
  if tostring(sourceId or "") ~= "velocity_mapper.voice" and specId ~= "velocity_mapper" then
    return nil
  end
...
function M.publishViewState(ctx)
  _G.__midiSynthVelocityMapperViewState = _G.__midiSynthVelocityMapperViewState or {}
  local runtime = ctx and ctx._dynamicVelocityMapperRuntime or nil
  ...
  for moduleId, state in pairs(runtime) do
    if type(state) == "table" then
      ...
      if inputAmp ~= nil then
        outputAmp = remapAmplitude(inputAmp, amount, curve, offset)
      end
      _G.__midiSynthVelocityMapperViewState[tostring(moduleId)] = {
        values = { amount = amount, curve = curve, offset = offset },
        inputAmp = inputAmp,
        outputAmp = outputAmp,
