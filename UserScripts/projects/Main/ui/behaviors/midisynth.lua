...
local OSC_MODE_OPTIONS = { "Classic", "Sample Loop" }
...
local function updateDropdownAnchors(ctx)
  ...
  anchorDropdown(all[rootId .. ".oscillatorComponent.sample_mode_dropdown"])
  anchorDropdown(all[rootId .. ".oscillatorComponent.sample_source_dropdown"])
...
local function isUiInteracting(ctx)
  ...
  local trackedSuffixes = {
    ...
    ".oscillatorComponent.sample_mode_dropdown",
    ".oscillatorComponent.sample_source_dropdown",
    ".oscillatorComponent.sample_bars_box",
    ".oscillatorComponent.sample_root_box",
    ".oscillatorComponent.sample_start_box",
    ".oscillatorComponent.sample_len_box",
    ".oscillatorComponent.sample_retrigger_toggle",
...
function M.init(ctx)
  ...
  local oscModeDrop = all[rootId .. ".oscillatorComponent.sample_mode_dropdown"]
  local oscSampleSourceDrop = all[rootId .. ".oscillatorComponent.sample_source_dropdown"]
  local oscSampleCaptureBtn = all[rootId .. ".oscillatorComponent.sample_capture_button"]
  local oscSampleBarsBox = all[rootId .. ".oscillatorComponent.sample_bars_box"]
  local oscSampleRootBox = all[rootId .. ".oscillatorComponent.sample_root_box"]
  local oscSampleStartBox = all[rootId .. ".oscillatorComponent.sample_start_box"]
  local oscSampleLenBox = all[rootId .. ".oscillatorComponent.sample_len_box"]
  local oscRangeViewDrop = all[rootId .. ".oscillatorComponent.range_view_dropdown"]
  ...
  if oscCtx then
    ...
    oscCtx._onVoiceRangeChange = function(voiceIdx, which, value)
      if which == "start" then
        setPath(string.format("/midi/synth/voice/%d/loopStart", voiceIdx), clamp(value, 0.0, 0.95))
        if oscSampleStartBox and oscSampleStartBox.setValue then
          oscSampleStartBox:setValue(math.floor(value * 100))
        end
      elseif which == "len" then
        setPath(string.format("/midi/synth/voice/%d/loopLen", voiceIdx), clamp(value, 0.05, 1.0))
        if oscSampleLenBox and oscSampleLenBox.setValue then
          oscSampleLenBox:setValue(math.floor(value * 100))
        end
      end
  ...
  if oscModeDrop and oscModeDrop.setOptions then
    oscModeDrop:setOptions(OSC_MODE_OPTIONS)
  end
  ...
  if oscModeDrop then oscModeDrop._onSelect = function(idx)
    setPath(PATHS.oscMode, idx - 1)
  end end
  ...
  if oscSampleStartBox then oscSampleStartBox._onChange = function(v)
    local rangeView = oscCtx and oscCtx.rangeView or "global"
    if rangeView:match("^voice%d$") then
      local voiceIdx = tonumber(rangeView:match("%d"))
      setPath(string.format("/midi/synth/voice/%d/loopStart", voiceIdx), clamp(v / 100.0, 0.0, 0.95))
    else
  ...
  if oscSampleLenBox then oscSampleLenBox._onChange = function(v)
    ...
    if rangeView:match("^voice%d$") then
      local voiceIdx = tonumber(rangeView:match("%d"))
      setPath(string.format("/midi/synth/voice/%d/loopLen", voiceIdx), clamp(v / 100.0, 0.05, 1.0))
    else
      setPath(PATHS.sampleLoopLen, clamp(v / 100.0, 0.05, 1.0))
    end
  end end

  -- Range view dropdown selection
  if oscRangeViewDrop then oscRangeViewDrop._onSelect = function(idx)
    if not oscCtx then return end
    local views = {"all", "global", "voice1", "voice2", "voice3", "voice4", "voice5", "voice6", "voice7", "voice8"}
    local selectedView = views[idx] or "all"
    oscCtx.rangeView = selectedView
    oscCtx.rangeViewIndex = idx
    
    -- Update Start/Len boxes to reflect selected view's values
    local startVal, lenVal
    if selectedView:match("^voice%d$") then
      local voiceIdx = tonumber(selectedView:match("%d"))
      local loopData = (type(getVoiceLoopData) == "function") and getVoiceLoopData() or {}
      local vData = loopData[voiceIdx] or {start = 0, len = 1}
      startVal = math.floor(vData.start * 100)
      lenVal = math.floor(vData.len * 100)
    else
      -- Global values
      startVal = math.floor((readParam(PATHS.sampleLoopStart, 0) or 0) * 100)
      lenVal = math.floor((readParam(PATHS.sampleLoopLen, 1) or 1) * 100)
    end
    if oscSampleStartBox and oscSampleStartBox.setValue then oscSampleStartBox:setValue(startVal) end
    if oscSampleLenBox and oscSampleLenBox.setValue then oscSampleLenBox:setValue(lenVal) end
...
function M.update(ctx, rawState)
  ...
  syncSelected(oscAll[rootId .. ".oscillatorComponent.sample_mode_dropdown"], oscMode + 1)
  syncSelected(oscAll[rootId .. ".oscillatorComponent.sample_source_dropdown"], sampleSource + 1)
  syncValue(oscAll[rootId .. ".oscillatorComponent.sample_bars_box"], sampleCaptureBars)
  syncValue(oscAll[rootId .. ".oscillatorComponent.sample_root_box"], sampleRootNote)
  syncValue(oscAll[rootId .. ".oscillatorComponent.sample_start_box"], sampleLoopStartPct)
  syncValue(oscAll[rootId .. ".oscillatorComponent.sample_len_box"], sampleLoopLenPct)
  syncToggleValue(oscAll[rootId .. ".oscillatorComponent.sample_retrigger_toggle"], sampleRetrigger)
