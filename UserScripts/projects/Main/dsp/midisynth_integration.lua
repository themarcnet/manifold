...
function M.buildSynth(ctx, options)
  ...
  local sampleMode = OSC_MODE_CLASSIC
  ...
  for i = 1, VOICE_COUNT do
    ...
    -- Keep direct bind for classic mode; sample mode overlays speed mapping.
  ...
  local function applySampleWindowToVoice(voice)
    ...
    local loopLength = capturedLength > 0 and capturedLength or (voice.samplePlayback:getLoopLength() or 0)
    if loopLength <= 0 then
      return
    end

    local trimmedLength = math.max(1, math.floor(loopLength * sampleLoopLen + 0.5))
    voice.samplePlayback:setLoopLength(trimmedLength)
    local seekPos = clamp(sampleLoopStart, 0.0, 0.999)
    voice.samplePlayback:seek(seekPos)
  end
    ...
    if sampleMode == OSC_MODE_SAMPLE_LOOP then
      if g > 0.5 then
        if sampleRetrigger then
          voice.samplePlayback:seek(clamp(sampleLoopStart, 0.0, 0.999))
        end
    ...
    for i = 1, VOICE_COUNT do
      ...
      if playbackNode then
        ...
        if ok and copied then
          ...
          -- Seek to start of newly captured sample first
