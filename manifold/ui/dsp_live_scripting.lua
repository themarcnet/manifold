...
local function findDefaultPresetIndex()
    for i, preset in ipairs(presets) do
        local path = string.lower(preset.path or "")
        local name = string.lower(preset.name or "")
        if path:find("default_dsp%.lua", 1, false) or name == "default_dsp" then
            return i
        end
    end
    return 1
...
-- Load presets at startup and prefer default_dsp.lua in editor
loadPresets()
state.selectedPreset = findDefaultPresetIndex()
...
local function refreshMetrics()
    state.graphInputRms = getParam("/manifold/debug/graphInputRms") or 0.0
    state.graphWetRms = getParam("/manifold/debug/graphWetRms") or 0.0
    state.graphMixedRms = getParam("/manifold/debug/graphMixedRms") or 0.0
    state.graphNodeCount = math.floor((getParam("/manifold/debug/graphNodeCount") or 0.0) + 0.5)
    state.graphRouteCount = math.floor((getParam("/manifold/debug/graphRouteCount") or 0.0) + 0.5)
...
function ui_init(rootNode)
    ...
    ui.refreshButton = W.Button.new(ui.rootPanel.node, "refresh", {
        ...
        on_click = function()
            ...
            ui.presetDropdown:setValue(state.selectedPreset)
            applyPresetToEditor(state.selectedPreset)
